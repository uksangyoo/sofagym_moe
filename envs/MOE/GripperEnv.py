# -*- coding: utf-8 -*-
"""Specific environment for the MOE gripper.
"""
...
class MOEGripperEnv:
    """Sub-class of AbstractEnv, dedicated to the MOE gripper scene.
    ...
    DEFAULT_CONFIG = {"scene": "MOE",
                      ...
                      "save_data": False,
                      "save_image": False,
                      "save_path": path + "/Results" + "/MOEGripper",
                      "planning": False,
                      "discrete": True,
                      "seed": None,
    ...
    def step(self, action):
        ...
        if self.use_server:
            obs = self.env.get_obs(self.env.config, self.nb_actions)
            state = np.array(obs['observation'], dtype=np.float32)
            reward = obs['reward']
            done = obs['done']
            info = obs['info']
            ...
            state = np.array(self.env._getState(self.env.root), dtype=np.float32)
            reward = self.env._getReward(self.env.root)
            done = self.env._getDone(self.env.root)
            info = self.env._getInfo(self.env.root)

        return state, reward, done, info

    def initialize_states(self):
        """Initialize the states of the environment.
        """
        self.env.init_states = self.env.config["init_states"]

    def init_goal(self):
        """Initialize the goal of the environment.
        """
        self.env.goal = self.env.config["goalList"]
