...
class FingerController(Sofa.Core.Controller):
    ...
    def onAnimateBeginEvent(self, event):
        ...
        if self.saveCount % 5 == 0:
            self.force_log[0].append(self.cable1.CableConstraint.force.value)
            self.force_log[1].append(self.cable2.CableConstraint.force.value)
            self.force_log[2].append(self.cable3.CableConstraint.force.value)
            self.force_log[3].append(self.cable4.CableConstraint.force.value)

            self.length_log[0].append(self.cable1.CableConstraint.cableLength.value)
            self.length_log[1].append(self.cable2.CableConstraint.cableLength.value)
            self.length_log[2].append(self.cable3.CableConstraint.cableLength.value)
            self.length_log[3].append(self.cable4.CableConstraint.cableLength.value)
    ...
    def saveLogs(self, directory=None):
        if directory is None:
            directory = os.path.join(self.log_dir, "tendon")
        os.makedirs(directory, exist_ok=True)

        np.savez(f"{directory}/tendon_logs.npz",
                force1=np.array(self.force_log[0]),
                force2=np.array(self.force_log[1]),
                force3=np.array(self.force_log[2]),
                force4=np.array(self.force_log[3]),
                L1=np.array(self.length_log[0]),
                L2=np.array(self.length_log[1]),
                L3=np.array(self.length_log[2]),
                L4=np.array(self.length_log[3]))
...
def Finger(parentNode=None, name="Finger",
           ...
           dir=None, T=None, T_dir=None, tendon_dir = None): 
    ...
    finger.addObject(FingerController(cable1, cable2, cable3, cable4, eobject, T_dir, tendon_dir))
...
def createScene(rootNode):
    ...
    root_dir = f'/media/moe/data/{exp_name}_{xi}_{yi}_{zi}_{ytheta}_{xtheta}_{timestamp}'.replace(".", "-")
    os.makedirs(root_dir + '/mesh')
    os.makedirs(root_dir + '/contact')


    tendon_dir = f'/media/moe/data/{exp_name}_{xi}_{yi}_{zi}_{ytheta}_{xtheta}_{timestamp}/force_and_length'.replace(".", "-")
    os.makedirs(tendon_dir)
    ...
    Finger(rootNode, 'finger1', rotation, [T1[0,3], T1[1,3], T1[2,3]], dir=root_dir, T=T, T_dir=T_dir, tendon_dir=tendon_dir)
    ...
    T1 = np.matmul(T, T0)
    rotation = R.from_matrix(T1[:3,:3]).as_euler('xyz', degrees=True)
    Finger(rootNode, 'finger2', rotation, [T1[0,3], T1[1,3], T1[2,3]], dir=root_dir, T=T, T_dir=T_dir, tendon_dir=tendon_dir)
