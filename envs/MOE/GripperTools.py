...
class rewardShaper(Sofa.Core.Controller):
    ...
    def getReward(self):
        ...
        """
        eff_y = self.effMO.position[0][1]
        goal_y = self.goal_pos[1]
        import numpy as np
        if isinstance(eff_y, (list, np.ndarray)):
            eff_y = eff_y[0]
        if isinstance(goal_y, (list, np.ndarray)):
            goal_y = goal_y[0]
        if hasattr(eff_y, "item"):
            eff_y = eff_y.item()
        if hasattr(goal_y, "item"):
            goal_y = goal_y.item()
        current_cost = float(abs(eff_y - goal_y))
        if self.cost is None:
            self.cost = current_cost
            return 0, self.cost
        if isinstance(self.cost, (list, np.ndarray)):
            self.cost = self.cost[0]
        if hasattr(self.cost, "item"):
            self.cost = self.cost.item()
        reward = float(self.cost) - current_cost
        self.cost = current_cost
...
class goalSetter(Sofa.Core.Controller):
    ...
    def update(self, goal):
        ...
        with self.goalMO.position.writeable() as position:
            import numpy as np
            arr = np.array(self.goalPos)
            
            # Handle the case where arr is 3x3 but we need 1x3
            if arr.shape == (3, 3):
                arr = arr[0, :]  # Take the first row
            
            if position.ndim == 2 and position.shape[0] > 0 and position.shape[1] == arr.shape[0]:
                position[0, :] = arr
            elif position.ndim == 1 and position.shape[0] == arr.shape[0]:
                position[:] = arr
            else:
                # fallback: try to broadcast or reshape
                try:
                    position[:] = arr.reshape(position.shape)
                except:
                    # Last resort: try to assign the first element
                    position[0] = arr
...
def getState(root):
    ...
    # Get the last vertex position as the tip (more robust than hardcoded index)
    finger_tips1 = [round(k, cs) for k in finger1.tetras.position.value[-1].tolist()]
    finger_tips1[0] /= 120
    finger_tips1[1] /= 140
    finger_tips1[2] /= 120

    finger_tips2 = [round(k, cs) for k in finger2.tetras.position.value[-1].tolist()]
    finger_tips2[0] /= 120
    finger_tips2[1] /= 140
    finger_tips2[2] /= 120
...
def getDone(root):
    ...
    """
    # Use the same logic as getReward to determine if done
    reward, cost = root.Reward.getReward()
...
def getInfo(root):
    ...
    """
    # Get current positions and other useful information
    finger1_pos = root.Gripper.Finger1.tetras.position.value.tolist()
    finger2_pos = root.Gripper.Finger2.tetras.position.value.tolist()
    cube_pos = root.Cube.mstate.position.value.tolist()
    goal_pos = _getGoalPos(root).tolist()
    
    info = {
        'finger1_position': finger1_pos,
        'finger2_position': finger2_pos,
        'cube_position': cube_pos,
        'goal_position': goal_pos,
        'cost': root.Reward.getReward()[1] if hasattr(root, 'Reward') else 0.0
    }
