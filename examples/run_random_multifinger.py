...
def main(config: Optional[dict] = None, max_steps: int = 20, visualize: bool = False, action_scale: float = 0.5):
    try:
        ...
        print("Could not import MultiFingerMOEEnv:", e)
    ...
    try:
        ...
        env = MultiFingerMOEEnv(config=cfg, use_server=False)
    except Exception as e:
        print("Failed to create MultiFingerMOEEnv instance:", e)
        return
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Run random actions in MultiFingerMOE with incremental force control")
